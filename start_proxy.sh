...
if pgrep -f "tg_ws_proxy.py" > /dev/null; then
	...
	echo "$(date +"%Y-%m-%d %H:%M:%S") - opened without proxy." >> ~/proxy_log.txt
    exit 0
fi

cd "$PROXY_DIR" || { echo "$(date +"%Y-%m-%d %H:%M:%S") - error PROXY_DIR" >> ~/proxy_log.txt; exit 1; }

TEMP_LOG=$(mktemp)
nohup python tg_ws_proxy.py > "$TEMP_LOG" 2>&1 &
echo "$(date +"%Y-%m-%d %H:%M:%S") - proxy started." >> ~/proxy_log.txt
sleep 1
...
if [ -n "$SECRET" ]; then
	...
	echo "$(date +"%Y-%m-%d %H:%M:%S") - telegram opened." >> ~/proxy_log.txt
    termux-wake-lock
fi